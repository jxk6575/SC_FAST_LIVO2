...
void LIVMapper::updateStateWithPGO()
{
  ...
  if (translationDiff > 1.0 || rotationDiff > 0.2) {
    std::cout << "[PGO] Pose difference too large, skipping update. Trans: " 
  ...
  std::cout << "[PGO] State updated with PGO optimization (corrected transform)" << std::endl;
}

void LIVMapper::rebuildLocalMapWithPGO()
{
  if (!pgo_integration_enable || !pgo_map_rebuild_enable || pgoKeyframes.poses.empty()) {
    return;
  }
