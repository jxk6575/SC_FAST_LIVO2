...
std::atomic_bool gtSAMgraphMade{false};
std::mutex mtxMap;
std::mutex mtxKeyframes;
extern std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> keyframeClouds;

pcl::PointCloud<pcl::PointXYZI>::Ptr globalMap(new pcl::PointCloud<pcl::PointXYZI>);

float mapviz_filter_size = 0.1f;
...
extern std::mutex mKF;
extern std::vector<pcl::PointCloud<PointType>::Ptr> keyframeLaserClouds;
extern std::vector<Pose6D> keyframePosesUpdated;
...
void initNoises( void )
{
    ...
    if (isam == nullptr) {
        ...
        params.relinearizeThreshold = 0.1;  // 根据需求调整参数
        params.relinearizeSkip = 1;
        isam = new gtsam::ISAM2(params);  // 关键：分配内存
    }
...
void pubPath( void )
{
    ...
    // 只有在有有效位姿时才发布
    if (odomAftPGO.pose.pose.position.x != 0 || odomAftPGO.pose.pose.position.y != 0 || odomAftPGO.pose.pose.position.z != 0) {
...
{
    mKF.lock(); 
    int estimate_size = isamCurrentEstimate.size();
    int poses_size = keyframePosesUpdated.size();  // 获取有效范围
    for (int node_idx=0; node_idx < int(isamCurrentEstimate.size()); node_idx++)
    {
        if (node_idx >= poses_size) {
            break;  // 超出范围则退出循环，避免越界
        }
...
void runISAM2opt() {
    // 先把共享图和初值“换”到本地变量，锁外再调用 isam->update()
    gtsam::NonlinearFactorGraph local_graph;
    gtsam::Values               local_values;

    {
        std::lock_guard<std::mutex> lk(mtxPosegraph);

        // 快速把 graph 交换出来：共享区立刻变空，本地拿到全部增量
        gtSAMgraph = local_graph;
        local_graph = gtsam::NonlinearFactorGraph();

        // Values 没有 swap接口，就复制一份到本地，然后把共享的清空
        for (const auto& kv : initialEstimate) {
            local_values.insert(kv.key, kv.value);
        }
        initialEstimate.clear();

        // ⚠️ 注意：这里就不要再对 gtSAMgraph/initialEstimate 做 resize(0)/clear() 了
    } // —— 出了作用域立即解锁，让 isam 在线性化时无并发写入

    if (!local_graph.empty() || !local_values.empty()) {
        isam->update(local_graph, local_values);
        // 根据你的参数多迭代几次
...
*/
bool loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr& nearKeyframes,
                           const int& key, const int& searchNum)
{
    if (!nearKeyframes) nearKeyframes.reset(new pcl::PointCloud<PointType>());
    nearKeyframes->clear();

    // 1) 拷贝需要访问的指针和姿态到局部变量，整个过程受同一把锁保护
    std::vector<pcl::PointCloud<PointType>::Ptr> localClouds;
    std::vector<Pose6D>                         localPoses;
    {
        std::lock_guard<std::mutex> lk(mKF);
        const int cloudSize = static_cast<int>(keyframeLaserClouds.size());
        if (cloudSize == 0) return false;

        int s = std::max(0, key - std::max(0, searchNum));
        int e = std::min(cloudSize - 1, key + std::max(0, searchNum));

        localClouds.reserve(e - s + 1);
        localPoses.reserve (e - s + 1);
        for (int i = s; i <= e; ++i) {
            // 只拷贝 shared_ptr 与位姿；真正的几何运算在锁外做
            localClouds.push_back(keyframeLaserClouds[i]);
            localPoses.push_back(keyframePosesUpdated[i]);
        }
    } // 解锁，让其他线程可以继续更新全局容器

    // 2) 做坐标变换与拼接（避免返回临时 shared_ptr；用局部变量承接）
    for (size_t i = 0; i < localClouds.size(); ++i) {
        const auto& srcPtr = localClouds[i];
        if (!srcPtr || srcPtr->empty()) continue;

        // 显式生成一个变换结果到局部对象，避免悬垂
        auto transformed = local2global(srcPtr, localPoses[i]);
        transformed->points.reserve(srcPtr->size());


        // 你自己的变换：把 local2global 改成填充传出参数的形式会更安全
        // 这里示例假设你能这样写：
        //   void local2global(const pcl::PointCloud<PointType>& in,
        //                     const Pose6D& pose, pcl::PointCloud<PointType>& out);
        transformed = local2global(srcPtr, localPoses[i]);


        // 拼接：使用 insert 避免越界/未构造写
        *nearKeyframes += *transformed;
    }
    if (nearKeyframes->points.empty()) return false;

    nearKeyframes->width  = static_cast<uint32_t>(nearKeyframes->points.size());
    nearKeyframes->height = 1;
    nearKeyframes->is_dense = false;

    // 3) 降采样（滤波器对象加独立互斥；输入输出分离）
    {
        std::lock_guard<std::mutex> lk(mICPFilter);
        pcl::PointCloud<PointType>::Ptr cloud_temp(new pcl::PointCloud<PointType>());
        downSizeFilterICP.setInputCloud(nearKeyframes);
        downSizeFilterICP.filter(*cloud_temp);
        nearKeyframes->swap(*cloud_temp); // 更快且安全
    }
    ...
    gtsam::Pose3 Pose6dTogtsamPose3(Pose6D pose)
{
    ...
    gtsam::Pose3 doICPVirtualRelative(const pcl::PointCloud<PointType>::Ptr& cloudFrom,
                                      const pcl::PointCloud<PointType>::Ptr& cloudTo,
                                      int idx1, int idx2);
    gtsam::Pose3 doICPVirtualRelative(const pcl::PointCloud<PointType>::Ptr& cloudFrom,
                                  const pcl::PointCloud<PointType>::Ptr& cloudTo,
                                  int idx1, int idx2)
{
    ...
    alignedMsg.header.frame_id = "camera_init";
    pubLoopScanLocalRegisted.publish(alignedMsg);

    // 检查结果
    if (!icp.hasConverged() || icp.getFitnessScore() > loopFitnessScoreThreshold) {
        std::cout << "[SC loop] ICP failed (" << icp.getFitnessScore() 
    ...
    Eigen::Affine3f correctionLidarFrame(correctionLidarFrame = Eigen::Affine3f(icp.getFinalTransformation()));
    Eigen::Affine3f tWrong = Pose6dToAffine3f(keyframePosesUpdated[idx1]);
    Eigen::Affine3f tCorrect = correctionLidarFrame * tWrong;
    pcl::getTranslationAndEulerAngles(tCorrect, x, y, z, roll, pitch, yaw);

    gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));
    gtsam::Pose3 poseTo   = Pose6dTogtsamPose3(keyframePosesUpdated[idx2]);
    return poseFrom.between(poseTo);
}
                                  
    gtsam::Pose3 doICPVirtualRelative(const int& _curr_kf_idx, const int& _loop_kf_idx)
{
    // parse pointclouds
...
void process_icp(void)
{
    while(1)
    {
		while ( !scLoopICPBuf.empty() )
        {
            ...
            auto cloudFromPtr = keyframeLaserClouds[prev_node_idx];
            auto cloudToPtr   = keyframeLaserClouds[curr_node_idx];

            
            pcl::PointCloud<PointType>::Ptr cloudFrom(new pcl::PointCloud<PointType>(*cloudFromPtr));
            pcl::PointCloud<PointType>::Ptr cloudTo(new pcl::PointCloud<PointType>(*cloudToPtr));

            auto relative_pose = doICPVirtualRelative(cloudFrom, cloudTo, prev_node_idx, curr_node_idx);
...
void process_isam() {
    const float hz = graphUpdateFrequency;
    ros::Rate rate(hz);
    while (ros::ok()) {
        rate.sleep();
        if (gtSAMgraphMade) {
            // 不再 mtxPosegraph.lock(); runISAM2opt 里自己处理并发
            runISAM2opt();
...
void pubMap() {
    ...
    // 2) 修正元数据（PCL 依赖 width/height/is_dense；不设好容易越界）
    input->width  = static_cast<uint32_t>(input->points.size());
    input->height = 1;
    input->is_dense = true;

    // 3) 体素滤波：输入 setInputCloud(input)，输出独立对象 down（绝对不要就地覆盖）
    pcl::VoxelGrid<pcl::PointXYZI> vg;
    vg.setLeafSize(mapviz_filter_size, mapviz_filter_size, mapviz_filter_size);
    ...
    // 4) 再次确认元数据正确
    down->width  = static_cast<uint32_t>(down->points.size());
    down->height = 1;
    down->is_dense = true;

    // 5) 发布
    sensor_msgs::PointCloud2 msg;
    pcl::toROSMsg(*down, msg);
    msg.header.frame_id = "map";          // 按你工程的坐标系改
    msg.header.stamp    = ros::Time::now();
    pubMapAftPGO.publish(msg);
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::PointCloud2>("/cloud_registered_local", 100, laserCloudFullResHandler);
